...
export const appSlice = createSlice({
    ...
    reducers: {
        ...
        setGame: (state, action) => {
            const currentGame = state.games.find((game) => game.threadId === action.payload.threadId);
            if (currentGame) {
        ...
        deleteGame: (state, action) => {
            state.games = state.games.filter((game) => game.threadId !== action.payload);
        },
        setThreadTitle: (state, action) => {
            const currentLog = state.chatLogs?.find((log) => log.threadId === action.payload.threadId);
            if (currentLog)
                currentLog.title = action.payload.title;
        },
        setChatLogs: (state, action) => {
            state.chatLogs = action.payload.map((cl) => ({
        ...
        deleteChatLog: (state, action) => {
            state.chatLogs = state.chatLogs.filter((log) => log.threadId !== action.payload);
        },
        ...
        addChatLog: (state, action) => {
            const currentLog = state.chatLogs?.find((log) => log.threadId === action.payload.threadId);
            if (currentLog) {
                if (!currentLog.chatLog)
                    currentLog.chatLog = [];
                if (!currentLog.type)
                    currentLog.type = action.payload.type;
                if (!currentLog.title)
                    currentLog.title =
                        action.payload.title ?? `New ${action.payload.type}`;
                // if last message is already from "narrator", we concatenate
                if (currentLog.chatLog[currentLog.chatLog.length - 1]?.role ===
                    "assistant" &&
        ...
        deleteCharacter: (state, action) => {
            state.characters = state.characters.filter((character) => character.threadId !== action.payload);
        },
        setCharacter: (state, action) => {
            const currentCharacter = state.characters?.find((character) => character.threadId === action.payload.threadId);
            if (currentCharacter) {
                ...
                if (action.payload.newImage) {
                    if (!currentCharacter.images)
                        ...
                        currentCharacter.images.unshift(action.payload.newImage);
                }
                if (action.payload.newImagesMultisize) {
                    if (!currentCharacter.imagesMultisize)
                        ...
                        currentCharacter.imagesMultisize.unshift(action.payload.newImagesMultisize);
                }
...
export const { setIsSmallScreen, toggleLeftPanel, toggleSettings, toggleLoginModal, setCurrentlyViewing, setSocketConnection, setThreadTitle, setChatLogs, setChatLog, addChatLog, setIsDataLoaded, setCharacter, setCharacters, deleteCharacter, deleteChatLog, setGame, deleteGame, } = appSlice.actions;
