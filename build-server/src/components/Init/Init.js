...
import { setSocketConnection, setIsDataLoaded, setIsSmallScreen, } from "../../store/features/app/appSlice";
import useWebSocket from "../../hooks/useWebSocket";
const WEBSOCKET_URL = "wss://rshu7p6m1f.execute-api.us-east-1.amazonaws.com/dev";
const RECONNECT_INTERVALS = [1000, 2000, 5000, 10000]; // Exponential backoff intervals (ms)
const Init = () => {
    const dispatch = useAppDispatch();
    const auth = useAuth();
    const { socketConnection, isDataLoaded } = useAppSelector((state) => state.app);
    ...
    window.addEventListener("resize", () => dispatch(setIsSmallScreen(window.innerWidth < 768)));
